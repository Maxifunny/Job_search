...
# Uruchamiany przez Azure Automation / Logic Apps lub ręcznie.
#
# Użycie ręczne:
...
log "=== Job Search daily run (Azure VM) ==="
log "Repo: $REPO_ROOT"

VENV_PYTHON="$REPO_ROOT/.venv/bin/python"
if [[ ! -x "$VENV_PYTHON" ]]; then
  log "ERROR: Brak .venv — uruchom infra/azure/install_vm.sh"
  exit 1
fi
...
chmod +x "$REPO_ROOT/infra/azure/run_daily_pipeline.sh" 2>/dev/null || true
